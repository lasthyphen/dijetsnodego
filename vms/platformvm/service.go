...
func (service *Service) ExportKey(r *http.Request, args *ExportKeyArgs, reply *ExportKeyReply) error {
	...
	address, err := djtx.ParseServiceAddress(service.addrManager, args.Address)
	if err != nil {
...
func (service *Service) GetBalance(_ *http.Request, args *GetBalanceRequest, response *GetBalanceResponse) error {
	...
	// Parse to address
	addrs, err := djtx.ParseServiceAddresses(service.addrManager, args.Addresses)
	if err != nil {
		return err
	}

	utxos, err := djtx.GetAllUTXOs(service.vm.state, addrs)
	if err != nil {
...
utxoFor:
	...
	response.Balance = response.Balances[service.vm.ctx.DJTXAssetID]
	response.Unlocked = response.Unlockeds[service.vm.ctx.DJTXAssetID]
	response.LockedStakeable = response.LockedStakeables[service.vm.ctx.DJTXAssetID]
	response.LockedNotStakeable = response.LockedNotStakeables[service.vm.ctx.DJTXAssetID]
	return nil
}
...
func (service *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, response *api.GetUTXOsReply) error {
	...
	addrSet, err := djtx.ParseServiceAddresses(service.addrManager, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = djtx.ParseServiceAddress(service.addrManager, args.StartIndex.Address)
		if err != nil {
...
func (service *Service) AddValidator(_ *http.Request, args *AddValidatorArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	fromAddrs, err := djtx.ParseServiceAddresses(service.addrManager, args.From)
	if err != nil {
		return err
	}

	// Parse the reward address
	rewardAddress, err := djtx.ParseServiceAddress(service.addrManager, args.RewardAddress)
	if err != nil {
	...
	if args.ChangeAddr != "" {
		changeAddr, err = djtx.ParseServiceAddress(service.addrManager, args.ChangeAddr)
		if err != nil {
...
func (service *Service) CreateSubnet(_ *http.Request, args *CreateSubnetArgs, response *api.JSONTxIDChangeAddr) error {
	...
	controlKeys, err := djtx.ParseServiceAddresses(service.addrManager, args.ControlKeys)
	if err != nil {
		return err
	}
...
func (service *Service) ImportDJTX(_ *http.Request, args *ImportDJTXArgs, response *api.JSONTxIDChangeAddr) error {
	...
	to, err := djtx.ParseServiceAddress(service.addrManager, args.To)
...
func (service *Service) getStakeHelper(tx *txs.Tx, addrs ids.ShortSet, totalAmountStaked map[ids.ID]uint64) []djtx.TransferableOutput {
...
func (service *Service) GetStake(_ *http.Request, args *GetStakeArgs, response *GetStakeReply) error {
	...
	response.Staked = response.Stakeds[service.vm.ctx.DJTXAssetID]
